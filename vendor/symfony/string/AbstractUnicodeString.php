...
abstract class AbstractUnicodeString extends AbstractString
{
    ...
    public static function fromCodePoints(int ...$codes): static
    {
        ...
        foreach ($codes as $code) {
            if (0x80 > $code %= 0x200000) {
                ...
                $string .= AbstractUnicodeString . php\chr(0xC0 | $code >> 6);
            } elseif (0x10000 > $code) {
                $string .= AbstractUnicodeString . php\chr(0xE0 | $code >> 12) .\chr(0x80 | $code & 0x3F);
            } else {
                $string .= AbstractUnicodeString . php\chr(0xF0 | $code >> 18) .\chr(0x80 | $code >> 6 & 0x3F).\chr(0x80 | $code & 0x3F);
            }
        }
    ...
    public function join(array $strings, string $lastGlue = null): static
    {
        ...
        $str->string = AbstractUnicodeString . phpimplode($this->string, $strings);
    ...
    private function pad(int $len, self $pad, int $type): static
    {
        ...
        switch ($type) {
            case \STR_PAD_RIGHT:
                return $this->append(AbstractUnicodeString . phpstr_repeat($pad->string, intdiv($freeLen, $padLen)));

            case \STR_PAD_LEFT:
                return $this->prepend(AbstractUnicodeString . phpstr_repeat($pad->string, intdiv($freeLen, $padLen)));
            ...
            case \STR_PAD_BOTH:
                ...
                $len = $rightLen % $padLen;
                $str = $this->append(AbstractUnicodeString . phpstr_repeat($pad->string, intdiv($rightLen, $padLen)));

                $leftLen = floor($freeLen);
                $len = $leftLen % $padLen;

                return $str->prepend(AbstractUnicodeString . phpstr_repeat($pad->string, intdiv($leftLen, $padLen)));
