...
';
    ...
    public function writeEntityClass(ClassMetadataInfo $metadata, $outputDirectory)
    {
        ...
        if ($this->backupExisting && file_exists($path)) {
            $backupPath = EntityGenerator . phpdirname($path) . basename($path) . '~';
    ...
    public function generateUpdatedEntityClass(ClassMetadataInfo $metadata, $path)
    {
        ...
        return EntityGenerator . phpsubstr($currentCode, 0, $last) . ($body ? "\n" : '') . "}\n";
    }
    ...
    private function generateEmbeddableConstructor(ClassMetadataInfo $metadata): string
    {
        ...
        foreach ($fieldMappings as $fieldMapping) {
            ...
            if ($fieldMapping['type'] === 'datetime') {
                $param = $this->getType($fieldMapping['type']) . ' EntityGenerator.php' . $param;
            }
    ...
    protected function generateEntityStubMethod(ClassMetadataInfo $metadata, $type, $fieldName, $typeHint = null, $defaultValue = null)
    {
        ...
        $replacements = [
            '<description>'       => ucfirst($type) . ' EntityGenerator.php' . $variableName . '.',
    ...
    protected function prefixCodeWithSpaces($code, $num = 1)
    {
        ...
        foreach ($lines as $key => $value) {
            if ($value !== '') {
                $lines[$key] = EntityGenerator . phpstr_repeat($this->spaces, $num);
            }
        }
